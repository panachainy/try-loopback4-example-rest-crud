...
export class BucketsRepository extends DefaultCrudRepository<
  ...
  BucketsRelations
> {
  public readonly bucketDetails: HasManyRepositoryFactory<
    BucketDetails,
    typeof Buckets.prototype.id
  >;
  ...
  constructor(
    ...
    @repository.getter('BucketDetailsRepository')
    protected bucketDetailsRepositoryGetter: Getter<BucketDetailsRepository>,
  ) {
    super(Buckets, dataSource);
    this.bucketDetails = this.createHasManyRepositoryFactoryFor(
      'bucketDetails',
      bucketDetailsRepositoryGetter,
    );
    this.registerInclusionResolver(
      'bucketDetails',
      this.bucketDetails.inclusionResolver,
    );
  }
}
